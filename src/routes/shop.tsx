...
function ShopPage() {
  ...
  useEffect(() => {
    ...
    else if (sort === "popular") query = query.order("reviews_count", { ascending: false });
    else query = query.order("created_at", { ascending: false });
    query.limit(200).then(({ data }) => setProducts((data as DBProduct[]) || []));
  }, [activeCat, q, sort, onSale]);
    ...
    <div className="min-h-screen pb-16 md:pb-0">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <button onClick={() => setOnSale(!onSale)} className={`px-4 py-1.5 text-sm border ${onSale ? "bg-gold text-gold-foreground border-gold" : "border-border"}`}>تخفيضات فقط</button>
        </div>
